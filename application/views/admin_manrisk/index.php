...
<head>
  ...
  <title>Menu Management Manrisk</title>
...
<body id="page-top">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <a href="" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#tambahmenuModal">Tambah Menu</a>
  ...
  <?php foreach ($admin_manrisk as $am) : ?>
    <tr>
      ...
      <td><?= $am['url']; ?></td>
      <td>
      <a href="" class="btn btn-warning btn-sm mb-1" data-toggle="modal" data-target="#editmenuManrisk<?= $am['id'];?>">Edit</a>
      <a href="" class="btn btn-danger btn-sm mb-1" data-toggle="modal" data-target="#hapusmenuManrisk<?= $am['id'];?>">Hapus</a>
    </tr>
    <?php endforeach; ?>
  </tbody>
</table>
...
<!-- Tambah Menu -->
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <form action="<?= base_url('admin_manrisk'); ?>" method="POST">
    ...
    <!-- Edit Menu -->
<p hidden><?= $nomor = 1; ?></p>
<?php foreach ($admin_manrisk as $am) : $nomor++; ?>
  <div class="modal fade" id="editmenuManrisk<?= $am['id'];?>" tabindex="-1" role="dialog" aria-labelledby="tambahmenuEditLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="tambahmenumodalLabel">Menu Edit</h5>
          <button class="close" type="button" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span>
          </button>
        </div>
        <form action="<?= base_url('admin_manrisk/edit'); ?>" method="POST">
        
        <div class="modal-body">
        <div class="form-group mb-3">
          <input type="hidden" name="id" value="<?= $am['id'];?>">
        <input type="text" class="form-control" id="nama" name="nama" value="<?= $am['nama'];?>" placeholder="nama">
        </div>
        <div class="form-group mb-3">
        <input type="text" class="form-control" id="icon" name="icon" value="<?= $am['icon'];?>" placeholder="Icon">
        </div>
        <div class="form-group mb-3">
        <input type="text" class="form-control" id="url" name="url" value="<?= $am['url'];?>" placeholder="URL">
        </div>
        </div>
        ...
        <div class="modal-footer">
          <button class="btn btn-secondary btn-sm" type="button" data-dismiss="modal">Cancel</button>
          <button type="submit" class="btn btn-primary btn-sm">Update</Tambah>
          </form>
        </div>
      </div>
    </div>
...
<p hidden><?= $nomor = 1; ?></p>
<?php foreach ($admin_manrisk as $am) : $nomor++; ?>
<div class="modal fade" id="hapusmenuManrisk<?= $am['id'];?>" tabindex="-1" role="dialog" aria-labelledby="hapusmenuManriskLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="hapusmenuManriskLabel">Hapus Data Menu</h5>
          <button class="close" type="button" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span>
          </button>
        </div>
        <form action="<?= base_url('admin_manrisk/hapus'); ?>" method="POST">
       ...
       Apakah anda yakin untuk menghapus data ini?
        ...
        <div class="modal-footer">
          <button class="btn btn-secondary btn-sm" type="button" data-dismiss="modal">Cancel</button>
          <button type="submit" class="btn btn-danger btn-sm">Hapus</Tambah>
          </form>
        </div>
      </div>
    </div>
